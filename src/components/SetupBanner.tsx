...
export default function SetupBanner() {
  ...
  return (
    <div id="supabase-setup-banner" className="bg-[#FAF8F5] border-b border-[#EADCC9] text-[#3E3C3A] text-sm relative z-50 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 py-3 sm:px-6 lg:px-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-start sm:items-center gap-3">
          <div className="p-2 bg-[#F3ECE0] rounded-lg text-[#C5A880] shrink-0">
            <Database className="w-5 h-5" />
          </div>
          <div>
            <p className="font-medium text-[#1F1E1D] flex items-center gap-1.5">
              <span>Supabase Live Connection Assistant</span>
              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-[#EADCC9] text-[#3E3C3A]">
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto">
          <button
            onClick={handleCopy}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-[#3E3C3A] text-white hover:bg-[#1F1E1D] transition-colors font-medium text-xs shadow-sm cursor-pointer"
          >
            {copied ? (
              <>
          ...
          <button
            ...
            className="p-1.5 rounded-md hover:bg-[#F3ECE0] text-[#8B7E74] transition-colors cursor-pointer"
